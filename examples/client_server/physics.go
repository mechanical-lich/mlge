...
// PhysicsSystem is a SimulationSystem that advances ball positions and
// bounces them off the world boundaries.
//
// It runs server-side only; the client never sees VelocityComponent.
type PhysicsSystem struct {
	// DT is the fixed timestep in seconds (1 / TickRate).
	DT float64
}

// Compile-time check that PhysicsSystem satisfies simulation.SimulationSystem.
