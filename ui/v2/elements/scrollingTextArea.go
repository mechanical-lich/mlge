...
func (s *ScrollingTextArea) Update() {
	// Mouse position
	mx, my := ebiten.CursorPosition()
	s.Focused = s.IsWithin(mx, my)
	if s.Parent != nil {
		parentX, parentY := s.Parent.GetPosition()
		mx -= parentX
		my -= parentY
	}

	absX, absY := s.GetAbsolutePosition()
	barX := absX + s.Width - 12
	barY := absY + 4
	barW := 32
	barH := s.Height - 8
...
func (s *ScrollingTextArea) Draw(screen *ebiten.Image, theme *theming.Theme) {
	absX, absY := s.GetAbsolutePosition()
	...
	if len(s.Lines) > s.VisibleLines {
		s.drawScrollbar(screen, theme)
	}
}

func (s *ScrollingTextArea) drawScrollbar(screen *ebiten.Image, theme *theming.Theme) {
	absX, absY := s.GetAbsolutePosition()
	barX := absX + s.Width - 12
	barY := absY + 4
	barW := 32
...
func (s *ScrollingTextArea) AddText(txt string) {
	...
	s.Lines = text.Wrap(s.Text, (s.Width-32)/5, 0) // Subtract padding for wrap

	// Drop oldest lines if over maxLines
