...
type GridContainer struct {
	...
	Children    []elements.ElementInterface // ordered children
...
func (g *GridContainer) AddChild(child elements.ElementInterface) {
	child.SetParent(g)
...
func (g *GridContainer) GetChild(name string) elements.ElementInterface {
...
func (g *GridContainer) Draw(screen *ebiten.Image, theme *theming.Theme) {
	ox, oy := g.GetAbsolutePosition()
	...
	for _, child := range g.Children {
		...
		if g.isVisible(cX, cY) {
			child.SetPosition(cX, cY)
			child.Draw(g.image, theme)
		}
	}

	g.Op.GeoM.Reset()
	g.Op.GeoM.Translate(float64(g.X), float64(g.Y))
...
func (g *GridContainer) GetAbsolutePosition() (int, int) {
	return 0, 0
}
